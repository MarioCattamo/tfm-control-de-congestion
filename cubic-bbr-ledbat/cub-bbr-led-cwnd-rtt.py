...
# Ejemplo de ejecucion ./cub-bbr-led-cwnd-rtt.py file-name
...
data3= np.genfromtxt(CAPTURE_FOLDER+FILE_NAME+'-cubic.csv', delimiter='\t')
...
zwindow2 = [(row[14]/nanosec_scale) for row in data2] # RTT
# Axis time adjustment
xtime2 = [elem-xtime2[0] for elem in xtime2]

# CUBIC
xtime3 = [row[0] for row in data3]
ywindow3 = [row[6] for row in data3]
zwindow3 = [row[9] for row in data3]
...
plt.plot(xtime3,ywindow3,label='CUBIC cwnd',color='green')
plt.title('CWND')
...
plt.plot(xtime3,zwindow3,label='CUBIC rtt',color='green')
plt.title('RTT')
plt.xlabel('time[seconds]')
plt.ylabel('ms')
plt.grid()
