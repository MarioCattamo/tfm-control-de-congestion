...
generate_throughput_data (){
	...
	ssh S2 "echo it | sudo -S tc qdisc replace dev eth0 root netem delay $MY_DELAY"
	...
	# Conexion a S2
	echo 'Listening in S2 port 49100...'
	ssh S2 "dd if=/proc/net/tcpprobe ibs=128 obs=128 | tee /tmp/$FILE_NAME-$MECHANISM-$BOTTLENECK_RATE.dat > /dev/null 2>&1" &
	ssh S2 "dd if=/dev/zero bs=1M count=100000000 | sudo nc -l 49100 2>&1 &" &

	sleep 5

	# Iniciando descarga C2<--S2
	echo 'Starting download C2:49000<--S2:49100...'
	ssh C2 "nc -dp 49000 S2 49100 > /dev/null &"
	...
	ssh -t S1 "echo it | sudo -S scp /tmp/$FILE_NAME-$MECHANISM-$BOTTLENECK_RATE.dat ledbat@C1:/home/ledbat/Documents/100325165/$FILE_NAME-$MECHANISM-$BOTTLENECK_RATE-S1.dat"

	echo 'Copiando .dat C2<--S2'
	ssh -t S2 "echo it | sudo -S scp /tmp/$FILE_NAME-$MECHANISM-$BOTTLENECK_RATE.dat ledbat@C1:/home/ledbat/Documents/100325165/$FILE_NAME-$MECHANISM-$BOTTLENECK_RATE-S2.dat"
